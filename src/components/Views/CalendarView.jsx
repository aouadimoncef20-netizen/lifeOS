    ...
    <div className="flex-1 overflow-y-auto flex flex-col gap-3 pr-1">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button className="px-3 py-1.5 rounded-lg border border-themed bg-transparent text-themed-secondary text-sm cursor-pointer transition-all duration-150 hover:bg-themed-surface hover:text-themed"
            onClick={() => { if (month === 0) { setMonth(11); setYear(y => y - 1); } else setMonth(m => m - 1); }}>←</button>
          <span className="text-base font-bold tracking-tight text-themed">{MONTHS[month]} {year}</span>
          <button className="px-3 py-1.5 rounded-lg border border-themed bg-transparent text-themed-secondary text-sm cursor-pointer transition-all duration-150 hover:bg-themed-surface hover:text-themed"
            onClick={() => { if (month === 11) { setMonth(0); setYear(y => y + 1); } else setMonth(m => m + 1); }}>→</button>
        </div>
        <div className="flex items-center gap-2">
          <button className="text-[11px] px-3 py-1 rounded-full border border-themed bg-transparent text-themed-secondary cursor-pointer transition-all duration-150 hover:border-accent-blue hover:text-accent-blue"
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {calDays.map((d, i) => {
          ...
          return (
            <div key={i}
              className={`min-h-[80px] rounded-lg border p-1.5 cursor-pointer flex flex-col gap-0.5 transition-all duration-150 ${
